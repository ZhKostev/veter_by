...
[['Свирь', 54.84821, 26.40015], ['Несвиж', 53.21735, 26.67815], ['Будслав', 54.785786, 27.45632],
 ['Лебедево', 54.31263, 26.71068], ['Замостье', 53.20808, 27.59319]].each do |t_coordinate|
  if WindGenerator.where(:title => t_coordinate[0]).empty?
    ...
    wg = WindGenerator.create!(:title => t_coordinate[0],
                               ...
                               :gmaps => true,
                               :region => minsk_reg)
    wg.photo = File.open("#{Rails.root}/public/images/fake_imgs/#{rand(8) + 1}.jpg")
    wg.save!
  end
end

[['Любча', 53.75275, 26.05502], ['Жировичи', 53.01307, 25.34486], ['Мир', 53.452, 26.46984],
 ['Гродно', 53.683301, 23.834624]].each do |t_coordinate|
  if WindGenerator.where(:title => t_coordinate[0]).empty?
